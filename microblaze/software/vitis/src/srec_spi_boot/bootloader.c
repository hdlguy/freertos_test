...
void xspidump(XSpi *xspiptr)
{
    ...
    xil_printf("HasFifos: 0x%08x\n\r", xspiptr->HasFifos);

    xil_printf("SlaveOnly: 0x%08x\n\r", xspiptr->SlaveOnly);
    xil_printf("NumSlaveBits: 0x%08x\n\r", xspiptr->NumSlaveBits);
    xil_printf("DataWidth: 0x%08x\n\r", xspiptr->DataWidth);
    xil_printf("SpiMode: 0x%08x\n\r", xspiptr->SpiMode);
    xil_printf("SlaveSelectMask: 0x%08x\n\r", xspiptr->SlaveSelectMask);
    xil_printf("SlaveSelectReg: 0x%08x\n\r", xspiptr->SlaveSelectReg);
    xil_printf("FlashBaseAddr: 0x%08x\n\r", xspiptr->FlashBaseAddr);
    xil_printf("XipMode: 0x%08x\n\r", xspiptr->XipMode);
    xil_printf("FifosDepth: 0x%08x\n\r", xspiptr->FifosDepth);
    xil_printf("*******************\n\r\n\r");
...
static uint8_t load_exec ()
{
	...
	while (!done) {
		for (int i=0; i<SREC_MAX_BYTES; i++) sr_buf[i]=0; // clear the buffer each time
...
static uint8_t flash_get_srec_line (uint8_t *buf)
{
	...
	Status = XSpi_Transfer(&Spi, WriteBuffer, ReadBuffer, (RECORD_TYPE + BYTE_COUNT + mode));
	if(Status != XST_SUCCESS) {
		return XST_FAILURE;
	}
